...
module packh_decode_ex_tb;
    ...
    wire id_reg_wen_o, id_use_rs1_o, id_use_rs2_o, id_ex_is_op_reg_o, id_ex_is_packh_o;
    wire [31:0] p_inst_o, p_inst_addr_o, p_op1_o, p_op2_o;
    wire [4:0] p_rd_addr_o;
    wire p_reg_wen_o, p_ex_is_op_reg_o, p_ex_is_packh_o;
    ...
    function automatic [31:0] make_packh;
        begin
            make_packh = {7'b0000100, 5'd7, 5'd6, 3'b111, 5'd5, 7'b0110011};
        end
    endfunction
    ...
    id u_id (
        ...
        .ex_is_op_reg_o(id_ex_is_op_reg_o), .ex_is_packh_o(id_ex_is_packh_o)
    );
    ...
    id_ex u_id_ex (
        ...
        .ex_is_packh_i(id_ex_is_packh_o),
    ...
    ex u_ex (
        ...
        .dec_func3_i(3'b111), .dec_func7_bit5_i(1'b0), .dec_func7_is_r_i(1'b0),
        ...
        .dec_is_csr_op_i(1'b0), .dec_is_packh_i(p_ex_is_packh_o),
    ...
    task automatic run_case(input [31:0] value1, input [31:0] value2);
        begin
            ...
            flush_flag = 1'b0;
            tick();
            expected = {16'b0, value2[7:0], value1[7:0]};
            if (!p_ex_is_packh_o || !ex_rd_wen_o || ex_rd_addr_o != 5'd5 || ex_rd_data_o !== expected) begin
                $error("packh failed rs1=%h rs2=%h got=%h expected=%h", value1, value2, ex_rd_data_o, expected);
                $finish;
            end
        end
    endtask
    ...
    initial begin
        ...
        inst = make_packh(); rs1_data = 32'b0; rs2_data = 32'b0;
        repeat (2) tick(); rst = 1'b1;

        #1;
        if (!id_ex_is_packh_o || !id_use_rs1_o || !id_use_rs2_o || !id_reg_wen_o ||
            id_rd_addr_o != 5'd5 || id_rs1_addr != 5'd6 || id_rs2_addr != 5'd7) begin
            $error("packh decode failed");
            $finish;
        end
        inst = {7'b0000101, 5'd7, 5'd6, 3'b111, 5'd5, 7'b0110011};
        #1;
        if (id_ex_is_packh_o) begin
            $error("neighboring funct7 decoded as packh");
            $finish;
        end

        inst = make_packh(); rs1_data = 32'h1111_22aa; rs2_data = 32'h3333_4455; tick();
        inst = make_packh(); rs1_data = 32'haaaa_bbbb; rs2_data = 32'hcccc_dddd; hold_flag = 1'b1; tick();
        if (!p_ex_is_packh_o || p_op1_o != 32'h1111_22aa || p_op2_o != 32'h3333_4455) begin
            $error("id_ex hold failed");
            $finish;
        end
        hold_flag = 1'b0; flush_flag = 1'b1; tick();
        if (p_ex_is_packh_o) begin
        ...
        run_case(32'haaaa_aaaa, 32'h1234_5678);
        for (i = 0; i < 500; i = i + 1) begin
            a = {$random, $random};
            b = {$random, $random};
            run_case(a, b);
        end
        $display("PASS: packh decode/id_ex/ex tests");
        $finish;
    end
endmodule
