...
module bexti_decode_ex_tb;
    ...
    function automatic [31:0] make_bexti(input [4:0] shamt);
        begin
            make_bexti = {7'b0100100, shamt, 5'd6, 3'b101, 5'd5, 7'b0010011};
        end
    endfunction

    function automatic [31:0] ref_bexti(input [31:0] value, input [4:0] shamt);
        begin
            ref_bexti = {31'b0, value[shamt]};
        end
    endfunction
    ...
    id u_id (
        ...
        .ex_is_bexti_o(id_ex_is_bexti_o)
    );
    ...
    id_ex u_id_ex (
        ...
        .ex_is_bexti_i(id_ex_is_bexti_o),
        .inst_o(p_inst_o),
    ...
    ex u_ex (
        ...
        .dec_is_bexti_i(p_ex_is_bexti_o),
    ...
    task automatic run_case(input [31:0] value, input [4:0] shamt);
        begin
            ...
            expected = ref_bexti(value, shamt);
            if (!p_ex_is_bexti_o || !ex_rd_wen_o || ex_rd_addr_o != 5'd5 ||
                ex_rd_data_o !== expected) begin
                $error("bexti failed value=%h shamt=%0d got=%h expected=%h valid=%b wen=%b rd=%0d",
                       value, shamt, ex_rd_data_o, expected, p_ex_is_bexti_o,
    ...
    initial begin
        ...
        hold_flag = 1'b0;
        flush_flag = 1'b0;
        inst = make_bexti(5'd0);
        rs1_data = 32'b0;
        rs2_data = 32'b0;
        repeat (2) tick();
        rst = 1'b1;

        inst = make_bexti(5'd31);
        rs1_data = 32'h8000_0000;
        #1;
        if (!id_ex_is_bexti_o || !id_use_rs1_o || id_use_rs2_o ||
            !id_reg_wen_o || id_rd_addr_o != 5'd5 ||
            id_rs1_addr != 5'd6 || id_op2_o != 32'd31) begin
            $error("bexti decode failed");
            $finish;
        end

        inst = {7'b0100101, 5'd31, 5'd6, 3'b101, 5'd5, 7'b0010011};
        #1;
        if (id_ex_is_bexti_o) begin
            $error("reserved neighboring funct7 decoded as bexti");
            $finish;
        end

        inst = make_bexti(5'd7);
        rs1_data = 32'h0000_0080;
        tick();
        inst = make_bexti(5'd3);
        rs1_data = 32'hffff_ffff;
        hold_flag = 1'b1;
        tick();
        if (!p_ex_is_bexti_o || p_op2_o != 32'd7 || p_op1_o != 32'h0000_0080) begin
        ...
        flush_flag = 1'b1;
        tick();
        if (p_ex_is_bexti_o) begin
            $error("id_ex flush failed to clear bexti valid");
            $finish;
        end
        flush_flag = 1'b0;
        ...
        $display("PASS: bexti decode/id_ex/ex tests");
        $finish;
    end
endmodule
