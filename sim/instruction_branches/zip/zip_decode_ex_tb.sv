...
module zip_decode_ex_tb;
    ...
    wire id_reg_wen_o, id_use_rs1_o, id_use_rs2_o, id_ex_is_op_imm_o, id_ex_is_zip_o;
    wire [31:0] p_inst_o, p_inst_addr_o, p_op1_o, p_op2_o;
    wire [4:0] p_rd_addr_o;
    wire p_reg_wen_o, p_ex_is_op_imm_o, p_ex_is_zip_o;
    ...
    function automatic [31:0] make_zip;
        begin
            make_zip = {12'h08f, 5'd6, 3'b001, 5'd5, 7'b0010011};
        end
    endfunction
    ...
    id u_id (
        ...
        .ex_is_op_imm_o(id_ex_is_op_imm_o), .ex_is_zip_o(id_ex_is_zip_o)
    );
    ...
    ex u_ex (
        ...
        .dec_is_csr_op_i(1'b0), .dec_is_zip_i(p_ex_is_zip_o),
    ...
    task automatic run_case(input [31:0] value);
        begin
            inst = make_zip(); rs1_data = value; rs2_data = 32'hxxxx_xxxx;
            hold_flag = 1'b0; flush_flag = 1'b0; tick();
            expected = ref_zip(value);
            if (!p_ex_is_zip_o || !ex_rd_wen_o || ex_rd_addr_o != 5'd5 || ex_rd_data_o !== expected) begin
                $error("zip failed value=%h got=%h expected=%h", value, ex_rd_data_o, expected);
                $finish;
            end
        end
    endtask
    ...
    initial begin
        ...
        inst = make_zip(); rs1_data = 32'b0; rs2_data = 32'b0;
        repeat (2) tick(); rst = 1'b1;
        #1;
        if (!id_ex_is_zip_o || !id_use_rs1_o || id_use_rs2_o || !id_reg_wen_o ||
            id_rd_addr_o != 5'd5 || id_rs1_addr != 5'd6) begin
            $error("zip decode failed");
            $finish;
        end
        inst = {12'h08e, 5'd6, 3'b001, 5'd5, 7'b0010011}; #1;
        if (id_ex_is_zip_o) begin
            $error("neighboring imm decoded as zip");
            $finish;
        end
        inst = make_zip(); rs1_data = 32'h0000_ffff; tick();
        inst = make_zip(); rs1_data = 32'hffff_0000; hold_flag = 1'b1; tick();
        if (!p_ex_is_zip_o || p_op1_o != 32'h0000_ffff) begin
            $error("id_ex hold failed");
            $finish;
        end
        hold_flag = 1'b0; flush_flag = 1'b1; tick();
        if (p_ex_is_zip_o) begin
        ...
        $display("PASS: zip decode/id_ex/ex tests");
        $finish;
    end
endmodule
