...
module orn_decode_ex_tb;
    ...
    // orn 标准 R 型编码：funct7=0100000, funct3=110, opcode=0110011。
    localparam [31:0] ORN_X5_X6_X7 = {7'b0100000, 5'd7, 5'd6, 3'b110, 5'd5, 7'b0110011};
    localparam [31:0] BAD_FUNCT7   = {7'b0100001, 5'd7, 5'd6, 3'b110, 5'd5, 7'b0110011};
    ...
    id_ex pipe_dut (
        ...
        .hold_flag_i(hold),
        .flush_flag_i(flush),
        .ex_is_orn_i(id_ex_is_orn_o),
        .ex_is_orn_o(p_ex_is_orn_o)
    );
    ...
    task automatic run_case(input [31:0] op1, input [31:0] op2);
        ...
        begin
            ...
            @(posedge clk);
            #1;
            check(ex_rd_data_o == expected_result, "orn result mismatch");
            check(ex_jump_en_o == 1'b0, "orn must not redirect PC");
        end
    endtask
    ...
    initial begin
        ...
        check(id_ex_is_orn_o == 1'b1, "ID must recognize orn");
        check(id_use_rs1_o == 1'b1, "orn must use rs1");
        check(id_use_rs2_o == 1'b1, "orn must use rs2");
        check(id_rd_addr_o == 5'd5, "orn rd decode");
        check(id_reg_wen == 1'b1, "orn must write rd");

        inst_i = BAD_FUNCT7;
        #1;
        check(id_ex_is_orn_o == 1'b0, "reserved adjacent funct7 must not decode as orn");
        inst_i = ORN_X5_X6_X7;

        @(posedge clk);
        #1;
        check(p_ex_is_orn_o == 1'b1, "ID/EX must latch orn valid");
        check(ex_rd_wen_o == 1'b1, "EX must write orn rd");
        check(ex_rd_addr_o == 5'd5, "EX rd address");
        check(ex_rd_data_o == (32'h00ff_0000 | ~32'h0f0f_0f0f), "EX orn result");

        hold = 1'b1;
        inst_i = `INST_NOP;
        @(posedge clk);
        #1;
        check(p_ex_is_orn_o == 1'b1, "hold must keep orn valid");

        hold = 1'b0;
        flush = 1'b1;
        @(posedge clk);
        #1;
        check(p_ex_is_orn_o == 1'b0, "flush must clear orn valid");
        flush = 1'b0;
        ...
        $display("PASS: orn decode/id_ex/ex tests");
        $finish;
    end
endmodule
