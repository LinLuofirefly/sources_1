...
module clmul_decode_ex_tb;
    ...
    wire id_reg_wen_o, id_use_rs1_o, id_use_rs2_o, id_ex_is_op_reg_o, id_ex_is_clmul_o;
    wire [31:0] p_inst_o, p_inst_addr_o, p_op1_o, p_op2_o;
    wire [4:0] p_rd_addr_o;
    wire p_reg_wen_o, p_ex_is_op_reg_o, p_ex_is_clmul_o;
    ...
    function automatic [31:0] make_clmul;
        begin
            make_clmul = {7'b0000101, 5'd7, 5'd6, 3'b001, 5'd5, 7'b0110011};
        end
    endfunction

    function automatic [31:0] ref_clmul(input [31:0] value1, input [31:0] value2);
        reg [63:0] acc;
        begin
            acc = 64'b0;
            for (k = 0; k < 32; k = k + 1)
    ...
    id u_id (
        ...
        .ex_is_op_reg_o(id_ex_is_op_reg_o), .ex_is_clmul_o(id_ex_is_clmul_o)
    );
    ...
    id_ex u_id_ex (
        ...
        .ex_is_clmul_i(id_ex_is_clmul_o),
    ...
    ex u_ex (
        ...
        .dec_func3_i(3'b001), .dec_func7_bit5_i(1'b0), .dec_func7_is_r_i(1'b0),
        ...
        .dec_is_csr_op_i(1'b0), .dec_is_clmul_i(p_ex_is_clmul_o),
    ...
    task automatic run_case(input [31:0] value1, input [31:0] value2);
        begin
            inst = make_clmul(); rs1_data = value1; rs2_data = value2;
            hold_flag = 1'b0; flush_flag = 1'b0;
            tick();
            inst = `INST_NOP; rs1_data = 32'b0; rs2_data = 32'b0;
            tick();

            expected = ref_clmul(value1, value2);
            cycles = 0;
            done_count = 0;
            while (cycles < 80 && done_count == 0) begin
                if (ex_rv32m_done_o) begin
                    ...
                    if (!ex_rd_wen_o || ex_rd_addr_o != 5'd5 || ex_rd_data_o !== expected) begin
                        $error("clmul failed rs1=%h rs2=%h got=%h expected=%h",
                               value1, value2, ex_rd_data_o, expected);
                        $finish;
                    end
                end
                tick();
            ...
            if (done_count != 1) begin
                $error("clmul did not produce exactly one done pulse, count=%0d", done_count);
                $finish;
            end
            if (ex_rv32m_done_o) begin
                $error("clmul done stayed high for more than one cycle");
                $finish;
            end
        end
    endtask
    ...
    initial begin
        ...
        inst = make_clmul(); rs1_data = 32'b0; rs2_data = 32'b0;
        repeat (2) tick(); rst = 1'b1;

        #1;
        if (!id_ex_is_clmul_o || !id_use_rs1_o || !id_use_rs2_o || !id_reg_wen_o ||
            id_rd_addr_o != 5'd5 || id_rs1_addr != 5'd6 || id_rs2_addr != 5'd7) begin
            $error("clmul decode failed");
            $finish;
        end
        inst = {7'b0000100, 5'd7, 5'd6, 3'b001, 5'd5, 7'b0110011}; #1;
        if (id_ex_is_clmul_o) begin
            $error("neighboring funct7 decoded as clmul");
            $finish;
        end

        flush_flag = 1'b1; tick();
        if (p_ex_is_clmul_o) begin
        ...
        $display("PASS: clmul decode/id_ex/ex iterative tests");
        $finish;
    end
endmodule
