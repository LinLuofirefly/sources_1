...
module xperm8_decode_ex_tb;
    ...
    wire id_reg_wen_o, id_use_rs1_o, id_use_rs2_o, id_ex_is_op_reg_o, id_ex_is_xperm8_o;
    wire [31:0] p_inst_o, p_inst_addr_o, p_op1_o, p_op2_o;
    wire [4:0] p_rd_addr_o;
    wire p_reg_wen_o, p_ex_is_op_reg_o, p_ex_is_xperm8_o;
    wire [4:0] ex_rd_addr_o;
    wire [31:0] ex_rd_data_o;
    wire ex_rd_wen_o, ex_rv32m_busy_o, ex_rv32m_done_o;
    integer i, lane, cycles, done_count;
    reg [31:0] a, b, expected;
    reg [7:0] idx;

    function automatic [31:0] make_xperm8;
        begin
            make_xperm8 = {7'b0010100, 5'd7, 5'd6, 3'b100, 5'd5, 7'b0110011};
        end
    endfunction

    function automatic [31:0] ref_xperm8(input [31:0] value1, input [31:0] value2);
        begin
            ref_xperm8 = 32'b0;
            for (lane = 0; lane < 4; lane = lane + 1) begin
                idx = (value2 >> (lane*8)) & 8'hff;
                if (idx < 4)
                    ref_xperm8 = ref_xperm8 | (((value1 >> (idx*8)) & 32'hff) << (lane*8));
            end
        end
    endfunction
    ...
    id u_id (
        ...
        .ex_is_op_reg_o(id_ex_is_op_reg_o), .ex_is_xperm8_o(id_ex_is_xperm8_o)
    );
    ...
    id_ex u_id_ex (
        ...
        .ex_is_xperm8_i(id_ex_is_xperm8_o),
        ...
        .ex_is_xperm8_o(p_ex_is_xperm8_o)
    );
    ...
    ex u_ex (
        ...
        .dec_func3_i(3'b100), .dec_func7_bit5_i(1'b0), .dec_func7_is_r_i(1'b0),
        ...
        .dec_is_csr_op_i(1'b0), .dec_is_xperm8_i(p_ex_is_xperm8_o),
    ...
    task automatic run_case(input [31:0] value1, input [31:0] value2);
        begin
            inst = make_xperm8(); rs1_data = value1; rs2_data = value2; hold_flag = 1'b0; flush_flag = 1'b0;
            tick(); inst = `INST_NOP; rs1_data = 32'b0; rs2_data = 32'b0; tick();
            expected = ref_xperm8(value1, value2); cycles = 0; done_count = 0;
            while (cycles < 24 && done_count == 0) begin
                if (ex_rv32m_done_o) begin
                    done_count = done_count + 1;
                    if (!ex_rd_wen_o || ex_rd_addr_o != 5'd5 || ex_rd_data_o !== expected) begin
                        $error("xperm8 failed rs1=%h rs2=%h got=%h expected=%h", value1, value2, ex_rd_data_o, expected);
                        $finish;
                    end
                end
            ...
            if (done_count != 1 || ex_rv32m_done_o) begin
                $error("xperm8 done pulse error count=%0d", done_count);
                $finish;
            end
        end
    endtask
    ...
    initial begin
        ...
        inst = make_xperm8(); rs1_data = 32'b0; rs2_data = 32'b0;
        repeat (2) tick(); rst = 1'b1;
        #1;
        if (!id_ex_is_xperm8_o || !id_use_rs1_o || !id_use_rs2_o || !id_reg_wen_o ||
            id_rd_addr_o != 5'd5 || id_rs1_addr != 5'd6 || id_rs2_addr != 5'd7) begin
            $error("xperm8 decode failed");
            $finish;
        end
        inst = {7'b0010101, 5'd7, 5'd6, 3'b100, 5'd5, 7'b0110011}; #1;
        if (id_ex_is_xperm8_o) begin
            $error("neighboring funct7 decoded as xperm8");
            $finish;
        end
        flush_flag = 1'b1; tick();
        if (p_ex_is_xperm8_o) begin
            $error("id_ex flush failed");
            $finish;
        end
        flush_flag = 1'b0;
        run_case(32'h3322_1100, 32'h0001_0203);
        run_case(32'h3322_1100, 32'h0302_0100);
        run_case(32'h3322_1100, 32'hffff_ffff);
        run_case(32'hffff_ffff, 32'h0000_0000);
        for (i = 0; i < 500; i = i + 1) begin
            a = {$random, $random};
            b = {$random, $random};
            run_case(a, b);
        end
        $display("PASS: xperm8 decode/id_ex/ex iterative tests");
        $finish;
    end
endmodule
