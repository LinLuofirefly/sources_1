...
module bseti_decode_ex_tb;
    logic clk = 1'b0, rst = 1'b0, hold = 1'b0, flush = 1'b0;
    always #5 clk = ~clk;

    // bseti 标准 OP-IMM 编码：funct7=0010100, funct3=001, opcode=0010011，shamt=inst[24:20]。
    localparam [31:0] BAD_FUNCT7 = {7'b0010101, 5'd31, 5'd6, 3'b001, 5'd5, 7'b0010011};

    logic [31:0] inst_i, rs1_data_i, op2_data_i;
    wire [4:0] id_rd_addr_o, ex_rd_addr_o;
    wire [31:0] ex_rd_data_o;
    wire id_reg_wen, id_use_rs1_o, id_use_rs2_o, id_ex_is_bseti_o, p_ex_is_bseti_o;
    wire ex_rd_wen_o, ex_jump_en_o;

    function automatic [31:0] make_bseti(input [4:0] sh);
        begin make_bseti = {7'b0010100, sh, 5'd6, 3'b001, 5'd5, 7'b0010011}; end
    endfunction
    ...
    id id_dut (.inst_i(inst_i), .inst_addr_i(32'h8000_1000), .rs1_data_i(rs1_data_i), .rs2_data_i(32'h0),
        ...
        .use_rs1_o(id_use_rs1_o), .use_rs2_o(id_use_rs2_o), .ex_is_bseti_o(id_ex_is_bseti_o));
    id_ex pipe_dut (.clk(clk), .rst(rst), .hold_flag_i(hold), .flush_flag_i(flush), .ex_is_bseti_i(id_ex_is_bseti_o), .ex_is_bseti_o(p_ex_is_bseti_o));
    ex ex_dut (.clk(clk), .rst(rst), .inst_i(inst_i), .inst_addr_i(32'h8000_1000),
        ...
        .dec_is_system_i(1'b0), .dec_is_rv32m_i(1'b0), .dec_is_csr_op_i(1'b0), .dec_is_bseti_i(p_ex_is_bseti_o),
    ...
    task automatic run_case(input [31:0] a, input [4:0] sh);
        logic [31:0] expected;
        begin
            rs1_data_i = a; inst_i = make_bseti(sh);
            expected = a | (32'b1 << sh);
            @(posedge clk); #1;
            check(ex_rd_data_o == expected, "bseti result mismatch");
            check(!ex_jump_en_o, "bseti must not redirect PC");
        end
    endtask

    integer i;
    initial begin
        rs1_data_i = 32'h0000_0000; inst_i = make_bseti(5'd31);
        repeat (2) @(posedge clk); rst = 1'b1; #1;
        check(id_ex_is_bseti_o, "ID must recognize bseti");
        check(id_use_rs1_o && !id_use_rs2_o && id_rd_addr_o == 5'd5 && id_reg_wen, "bseti decode metadata");
        inst_i = BAD_FUNCT7; #1; check(!id_ex_is_bseti_o, "bad funct7 must not decode as bseti");
        inst_i = make_bseti(5'd31); @(posedge clk); #1;
        check(p_ex_is_bseti_o && ex_rd_wen_o && ex_rd_addr_o == 5'd5, "EX bseti writeback metadata");
        check(ex_rd_data_o == 32'h8000_0000, "EX bseti result");
        hold = 1'b1; inst_i = `INST_NOP; @(posedge clk); #1; check(p_ex_is_bseti_o, "hold keeps bseti valid");
        hold = 1'b0; flush = 1'b1; @(posedge clk); #1; check(!p_ex_is_bseti_o, "flush clears bseti valid");
        flush = 1'b0;
        ...
        $display("PASS: bseti decode/id_ex/ex tests");
        $finish;
    end
endmodule
