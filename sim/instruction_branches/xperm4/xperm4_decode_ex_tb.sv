...
module xperm4_decode_ex_tb;
    ...
    wire id_reg_wen_o, id_use_rs1_o, id_use_rs2_o, id_ex_is_op_reg_o, id_ex_is_xperm4_o;
    wire [31:0] p_inst_o, p_inst_addr_o, p_op1_o, p_op2_o;
    wire [4:0] p_rd_addr_o;
    wire p_reg_wen_o, p_ex_is_op_reg_o, p_ex_is_xperm4_o;
    ...
    function automatic [31:0] make_xperm4;
        begin
            make_xperm4 = {7'b0010100, 5'd7, 5'd6, 3'b010, 5'd5, 7'b0110011};
        end
    endfunction

    function automatic [31:0] ref_xperm4(input [31:0] value1, input [31:0] value2);
        begin
            ref_xperm4 = 32'b0;
            for (lane = 0; lane < 8; lane = lane + 1) begin
                idx = (value2 >> (lane*4)) & 4'hf;
                if (idx < 8)
                    ref_xperm4 = ref_xperm4 | (((value1 >> (idx*4)) & 32'hf) << (lane*4));
            end
        end
    endfunction
    ...
    id u_id (
        ...
        .ex_is_op_reg_o(id_ex_is_op_reg_o), .ex_is_xperm4_o(id_ex_is_xperm4_o)
    );
    ...
    id_ex u_id_ex (
        ...
        .ex_is_xperm4_i(id_ex_is_xperm4_o),
        ...
        .ex_is_xperm4_o(p_ex_is_xperm4_o)
    );
    ...
    ex u_ex (
        ...
        .dec_func3_i(3'b010), .dec_func7_bit5_i(1'b0), .dec_func7_is_r_i(1'b0),
        ...
        .dec_is_csr_op_i(1'b0), .dec_is_xperm4_i(p_ex_is_xperm4_o),
    ...
    task automatic run_case(input [31:0] value1, input [31:0] value2);
        begin
            inst = make_xperm4(); rs1_data = value1; rs2_data = value2; hold_flag = 1'b0; flush_flag = 1'b0;
            tick(); inst = `INST_NOP; rs1_data = 32'b0; rs2_data = 32'b0; tick();
            expected = ref_xperm4(value1, value2); cycles = 0; done_count = 0;
            while (cycles < 32 && done_count == 0) begin
                if (ex_rv32m_done_o) begin
                    done_count = done_count + 1;
                    if (!ex_rd_wen_o || ex_rd_addr_o != 5'd5 || ex_rd_data_o !== expected) begin
                        $error("xperm4 failed rs1=%h rs2=%h got=%h expected=%h", value1, value2, ex_rd_data_o, expected);
                        $finish;
                    end
                end
            ...
            if (done_count != 1 || ex_rv32m_done_o) begin
                $error("xperm4 done pulse error count=%0d", done_count);
                $finish;
            end
        end
    endtask
    ...
    initial begin
        ...
        inst = make_xperm4(); rs1_data = 32'b0; rs2_data = 32'b0;
        repeat (2) tick(); rst = 1'b1;
        #1;
        if (!id_ex_is_xperm4_o || !id_use_rs1_o || !id_use_rs2_o || !id_reg_wen_o ||
            id_rd_addr_o != 5'd5 || id_rs1_addr != 5'd6 || id_rs2_addr != 5'd7) begin
            $error("xperm4 decode failed");
            $finish;
        end
        inst = {7'b0010101, 5'd7, 5'd6, 3'b010, 5'd5, 7'b0110011}; #1;
        if (id_ex_is_xperm4_o) begin
            $error("neighboring funct7 decoded as xperm4");
            $finish;
        end
        flush_flag = 1'b1; tick();
        if (p_ex_is_xperm4_o) begin
            $error("id_ex flush failed");
            $finish;
        end
        flush_flag = 1'b0;
        run_case(32'h7654_3210, 32'h0123_4567);
        run_case(32'h7654_3210, 32'h7777_7777);
        run_case(32'h7654_3210, 32'hffff_ffff);
        ...
        $display("PASS: xperm4 decode/id_ex/ex iterative tests");
        $finish;
    end
endmodule
