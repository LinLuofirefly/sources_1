...
module brev8_decode_ex_tb;
    ...
    function automatic [31:0] make_brev8;
        begin
            make_brev8 = {12'h687, 5'd6, 3'b101, 5'd5, 7'b0010011};
        end
    endfunction

    function automatic [31:0] ref_brev8(input [31:0] value);
        begin
            ref_brev8 = 32'b0;
            for (b = 0; b < 4; b = b + 1)
                for (j = 0; j < 8; j = j + 1)
                    ref_brev8[b*8 + j] = value[b*8 + (7-j)];
        end
    endfunction
    ...
    id u_id (
        ...
        .ex_is_brev8_o(id_ex_is_brev8_o)
    );
    ...
    id_ex u_id_ex (
        ...
        .ex_is_brev8_i(id_ex_is_brev8_o),
        .inst_o(p_inst_o),
    ...
    ex u_ex (
        ...
        .dec_is_brev8_i(p_ex_is_brev8_o),
    ...
    task automatic run_case(input [31:0] value);
        begin
            ...
            flush_flag = 1'b0;
            tick();
            expected = ref_brev8(value);
            if (!p_ex_is_brev8_o || !ex_rd_wen_o || ex_rd_addr_o != 5'd5 ||
                ex_rd_data_o !== expected) begin
                $error("brev8 failed value=%h got=%h expected=%h", value, ex_rd_data_o, expected);
                $finish;
            end
        end
    endtask
    ...
    initial begin
        ...
        inst = make_brev8();
        rs1_data = 32'h0123_4567;
        #1;
        if (!id_ex_is_brev8_o || !id_use_rs1_o || id_use_rs2_o ||
            !id_reg_wen_o || id_rd_addr_o != 5'd5 || id_rs1_addr != 5'd6) begin
            $error("brev8 decode failed");
            $finish;
        end

        inst = {12'h686, 5'd6, 3'b101, 5'd5, 7'b0010011};
        #1;
        if (id_ex_is_brev8_o) begin
            $error("neighboring imm decoded as brev8");
            $finish;
        end

        inst = make_brev8();
        rs1_data = 32'h0000_00f0;
        tick();
        inst = make_brev8();
        rs1_data = 32'hffff_ffff;
        hold_flag = 1'b1;
        tick();
        if (!p_ex_is_brev8_o || p_op1_o != 32'h0000_00f0) begin
        ...
        flush_flag = 1'b1;
        tick();
        if (p_ex_is_brev8_o) begin
            $error("id_ex flush failed to clear brev8 valid");
            $finish;
        end
        flush_flag = 1'b0;
        ...
        $display("PASS: brev8 decode/id_ex/ex tests");
        $finish;
    end
endmodule
