...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbs binv 语义：翻转 rs1 中由 rs2[4:0] 指定的位。
     // one-hot 掩码只服务写回结果，不接入普通 ALU 输入或 PC redirect 控制。
     wire [31:0] binv_mask_w = 32'b1 << alu_op2[4:0];
     wire [31:0] binv_result_w = alu_op1 ^ binv_mask_w;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_binv_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = binv_result_w;
                rd_wen_o  = rd_wen_i;
            end
