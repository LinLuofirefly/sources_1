...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbs bclri 语义：清除 rs1 中由立即数 shamt 指定的位。
     // shamt 已通过立即数操作数通道进入 EX，不再读取 inst_i[24:20]。
     wire [31:0] bclri_mask_w = 32'b1 << alu_op2[4:0];
     wire [31:0] bclri_result_w = alu_op1 & ~bclri_mask_w;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_bclri_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = bclri_result_w;
                rd_wen_o  = rd_wen_i;
            end
