...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb maxu 语义：rd = (rs1 > rs2) ? rs1 : rs2，比较按无符号数解释。
     // 局部比较结果只进入 rd_data_o 选择，不连接到分支或 PC 重定向控制。
     wire [31:0] maxu_result_w = (alu_op1 > alu_op2) ? alu_op1 : alu_op2;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_maxu_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = maxu_result_w;
                rd_wen_o  = rd_wen_i;
            end
