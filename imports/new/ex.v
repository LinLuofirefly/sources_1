...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb/Zbkb ror 语义：rd = rotate_right(rs1, rs2[4:0])。
     // 显式处理旋转量 0，避免移位 32 的边界歧义；结果只在写回路径局部选择。
     wire [4:0] ror_shamt_w = alu_op2[4:0];
     wire [4:0] ror_lshamt_w = 5'd0 - ror_shamt_w;
     wire [31:0] ror_result_w =
         (ror_shamt_w == 5'd0) ? alu_op1 :
         ((alu_op1 >> ror_shamt_w) | (alu_op1 << ror_lshamt_w));
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_ror_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = ror_result_w;
                rd_wen_o  = rd_wen_i;
            end
