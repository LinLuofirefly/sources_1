...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zba sh2add 语义：rd = rs2 + (rs1 << 2)。
     // EX 不再解析完整 inst_i，只使用 ID/EX 寄存后的 dec_is_sh2add_i。
     // 固定左移 2 位用拼接实现，避免生成可变移位器；结果只局部覆盖 rd_data_o，
     // 不接入分支比较、JALR 目标或 PC 重定向控制锥，因此不增加这些高扇出路径负载。
     wire [31:0] sh2add_result_w = alu_op2 + {alu_op1[29:0], 2'b00};
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_sh2add_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = sh2add_result_w;
                rd_wen_o  = rd_wen_i;
            end
