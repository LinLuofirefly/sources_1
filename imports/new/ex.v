...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbs bseti 语义：置位 rs1 中由立即数 shamt 指定的位。
     // shamt 来自已有立即数操作数通道，EX 不再读取 inst_i[24:20]。
     wire [31:0] bseti_mask_w = 32'b1 << alu_op2[4:0];
     wire [31:0] bseti_result_w = alu_op1 | bseti_mask_w;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_bseti_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = bseti_result_w;
                rd_wen_o  = rd_wen_i;
            end
