...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb sext.b 语义：rd = sign_extend(rs1[7:0])。
     // 采用固定拼接布线，不使用移位器或比较器；结果只在写回数据路径局部覆盖，
     // 不改变普通 ALU 输入，也不会接入分支/JALR/PC 重定向控制锥。
     wire [31:0] sext_b_result_w = {{24{alu_op1[7]}}, alu_op1[7:0]};
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_sext_b_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = sext_b_result_w;
                rd_wen_o  = rd_wen_i;
            end
