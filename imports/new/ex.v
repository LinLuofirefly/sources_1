...
module ex (
    ...
    wire clmul_start = (kill_i == 1'b0) && dec_is_clmul_i &&
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            if (clmul_start) begin
                // clmul 只处理新增指令自身：start 锁存源操作数和写回元数据，
                // busy 期间每拍处理 rs2 的一位，done 只拉高一拍，保证只写回一次。
                clmul_inst_r    <= inst_i;
                ...
                if (clmul_count_r == 6'd31) begin
                    clmul_busy_r   <= 1'b0;
                    clmul_done_r   <= 1'b1;
                    clmul_result_r <= clmul_acc_next_w[31:0];
                end else begin
