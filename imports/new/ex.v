...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbs bext 语义：rd = zero_extend(rs1[rs2[4:0]])。
     // 动态位选择只在该新增指令的写回结果路径使用，不改变普通 ALU 输入选择。
     wire [31:0] bext_result_w = {31'b0, alu_op1[alu_op2[4:0]]};
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_bext_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = bext_result_w;
                rd_wen_o  = rd_wen_i;
            end
