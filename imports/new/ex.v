...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbs binvi 语义：翻转 rs1 中由立即数 shamt 指定的位。
     // shamt 通过立即数操作数通道传入，掩码只在写回结果路径局部生成。
     wire [31:0] binvi_mask_w = 32'b1 << alu_op2[4:0];
     wire [31:0] binvi_result_w = alu_op1 ^ binvi_mask_w;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_binvi_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = binvi_result_w;
                rd_wen_o  = rd_wen_i;
            end
