...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb sext.h 语义：rd = sign_extend(rs1[15:0])。
     // 使用固定拼接布线，不生成通用移位/选择网络；结果只在写回数据处局部选择，
     // 不接入分支、JALR 或 PC 重定向控制锥。
     wire [31:0] sext_h_result_w = {{16{alu_op1[15]}}, alu_op1[15:0]};
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_sext_h_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = sext_h_result_w;
                rd_wen_o  = rd_wen_i;
            end
