...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb/Zbkb xnor 语义：rd = ~(rs1 ^ rs2)。
     // 新增 XOR/NOT 逻辑只在写回数据处被 dec_is_xnor_i 选择，
     // 不增加分支比较、JALR 目标或 PC redirect 网络的扇出。
     wire [31:0] xnor_result_w = ~(alu_op1 ^ alu_op2);
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_xnor_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = xnor_result_w;
                rd_wen_o  = rd_wen_i;
            end
