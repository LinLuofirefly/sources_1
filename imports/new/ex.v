...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb min 语义：rd = signed(rs1) < signed(rs2) ? rs1 : rs2。
     // 使用局部有符号比较器，不复用分支比较结果，避免把写回结果选择接入 PC redirect 关键路径。
     wire [31:0] min_result_w = ($signed(alu_op1) < $signed(alu_op2)) ? alu_op1 : alu_op2;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_min_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = min_result_w;
                rd_wen_o  = rd_wen_i;
            end
