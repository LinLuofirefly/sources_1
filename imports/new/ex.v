...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb/Zbkb andn 语义：rd = rs1 & ~rs2。
     // 这是纯布尔运算，EX 不再重新译码 inst_i；新增逻辑只驱动写回结果，
     // 不参与分支比较、JALR 目标生成或 PC 重定向，避免增加这些时序关键锥的扇出。
     wire [31:0] andn_result_w = alu_op1 & ~alu_op2;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_andn_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = andn_result_w;
                rd_wen_o  = rd_wen_i;
            end
