...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbb/Zbkb orn 语义：rd = rs1 | ~rs2。
     // 新增逻辑是局部布尔网络，只在 dec_is_orn_i 有效时覆盖 rd_data_o；
     // 不改变普通 ALU 输入，也不进入分支/JALR/PC 重定向控制锥。
     wire [31:0] orn_result_w = alu_op1 | ~alu_op2;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_orn_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = orn_result_w;
                rd_wen_o  = rd_wen_i;
            end
