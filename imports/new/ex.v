...
module ex (
    ...
    wire clmul_start = (kill_i == 1'b0) && dec_is_clmulh_i &&
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            if (clmul_start) begin
                // clmulh 使用同一套 GF(2) 迭代乘法器，done 时只写回高 32 位。
                // busy/done 复用现有 EX 多周期停顿语义，不改变原有指令 CPI。
                clmul_inst_r    <= inst_i;
                ...
                if (clmul_count_r == 6'd31) begin
                    clmul_busy_r   <= 1'b0;
                    clmul_done_r   <= 1'b1;
                    clmul_result_r <= clmul_acc_next_w[63:32];
                end else begin
