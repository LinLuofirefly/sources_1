...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbkb packh 语义：rd = {16'b0, rs2[7:0], rs1[7:0]}。
     // 固定拼接不增加通用 ALU 输入选择，也不接入分支/JALR 控制锥。
     wire [31:0] packh_result_w = {16'b0, alu_op2[7:0], alu_op1[7:0]};
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_packh_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = packh_result_w;
                rd_wen_o  = rd_wen_i;
            end
