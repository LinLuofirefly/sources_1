...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbkb pack 语义：rd = {rs2[15:0], rs1[15:0]}。
     // 这是固定拼接，不需要额外比较器/移位器；只在写回数据局部选择。
     wire [31:0] pack_result_w = {alu_op2[15:0], alu_op1[15:0]};
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_pack_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = pack_result_w;
                rd_wen_o  = rd_wen_i;
            end
