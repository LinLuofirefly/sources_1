...
module ex (
    ...
    output reg         bp_actual_taken_o,
     ...
     // Zbs bset 语义：置位 rs1 中由 rs2[4:0] 指定的位。
     // one-hot 掩码只在该新增指令的写回结果路径生成，不接入 PC 重定向控制锥。
     wire [31:0] bset_mask_w = 32'b1 << alu_op2[4:0];
     wire [31:0] bset_result_w = alu_op1 | bset_mask_w;
    ...
    always @(*) begin
        ...
        if (rv32m_busy_o == 1'b1) begin
            ...
            if (dec_is_bset_i) begin
                rd_addr_o = rd_addr_i;
                rd_data_o = bset_result_w;
                rd_wen_o  = rd_wen_i;
            end
