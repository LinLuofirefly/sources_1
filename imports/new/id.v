...
module id (
    ...
    wire is_zip         = (opcode == `INST_TYPE_I) &&
                          (func3 == 3'b001) &&
                          (imm == 12'h08f);
    ...
    // Zbkb zip 标准编码：opcode=0010011, funct3=001, imm[11:0]=0x08f。
    // zip 只读取 rs1，按固定布线交织低/高 16 位；EX 不再解析完整 inst_i。
    assign ex_is_zip_o              = is_zip;
