...
module id (
    ...
    wire is_unzip       = (opcode == `INST_TYPE_I) &&
                          (func3 == 3'b101) &&
                          (imm == 12'h08f);
    ...
    // Zbkb unzip 标准编码：opcode=0010011, funct3=101, imm[11:0]=0x08f。
    // unzip 是 zip 的逆固定映射；ID 只传递一个 valid，EX 不再重译码 inst_i。
    assign ex_is_unzip_o            = is_unzip;
