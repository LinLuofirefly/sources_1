...
module id (
    ...
    wire is_binvi       = (opcode == `INST_TYPE_I) &&
                          (func3 == 3'b001) &&
                          (func7 == 7'b0110100);
    ...
    // Zbs binvi 标准编码：opcode=0010011, funct3=001, funct7=0110100，shamt=inst[24:20]。
    // 立即数形式只依赖 rs1，EX 使用已传递的 shamt，不再解析完整 inst_i。
    assign ex_is_binvi_o            = is_binvi;
