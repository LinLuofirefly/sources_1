...
module id (
    ...
    wire is_maxu        = (opcode == `INST_TYPE_R_M) &&
                          (func3 == 3'b111) &&
                          (func7 == 7'b0000101);
    ...
    // Zbb maxu 标准编码：opcode=0110011, funct3=111, funct7=0000101。
    // 无符号比较只服务寄存器写回，避免复用或增加分支比较/PC redirect 关键锥负载。
    assign ex_is_maxu_o             = is_maxu;
