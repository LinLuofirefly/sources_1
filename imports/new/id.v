...
module id (
    ...
    wire is_clmul       = (opcode == `INST_TYPE_R_M) &&
                          (func3 == 3'b001) &&
                          (func7 == 7'b0000101);
    ...
    // Zbc clmul 标准编码：opcode=0110011, funct3=001, funct7=0000101。
    // 在 ID 精确译码为单独 valid，EX 迭代执行，避免单周期 32 路 XOR 部分积进入写回长路径。
    assign ex_is_clmul_o            = is_clmul;
