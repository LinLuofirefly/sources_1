...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_zip_o,
    ...
    // zip 是单周期固定布线指令。flush/reset 清零，hold 保持，
    // 保证 valid 与 rs1 数据一致，不影响原有指令 CPI。
    wire ex_is_zip_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_zip_o :
                       ex_is_zip_i;
