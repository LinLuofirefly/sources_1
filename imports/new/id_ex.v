...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_binv_o,
    ...
    // binv 是单周期单比特写回指令。flush/reset 清零，hold 保持，
    // 保证 valid 与操作数同步穿过 ID/EX。
    wire ex_is_binv_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_binv_o :
                       ex_is_binv_i;
