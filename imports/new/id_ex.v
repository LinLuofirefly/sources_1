...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_max_o,
    ...
    // max 是单周期比较选择写回指令。flush/reset 清零，hold 保持，
    // 保证 valid 与操作数在流水线停顿时不发生错位。
    wire ex_is_max_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_max_o :
                       ex_is_max_i;
