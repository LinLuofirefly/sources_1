...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_zext_h_o,
    ...
    // zext.h 是单周期固定布线写回指令。flush/reset 清零，hold 保持，
    // 使 valid 与 rs1 数据保持同拍，不会重复提交或丢失。
    wire ex_is_zext_h_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_zext_h_o :
                       ex_is_zext_h_i;
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            ex_is_zext_h_o  <= ex_is_zext_h_next;
