...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_pack_o,
    ...
    // pack 是单周期固定拼接指令。flush/reset 清零，hold 保持，
    // 保证 valid 与两个源操作数一起停顿或前进。
    wire ex_is_pack_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_pack_o :
                       ex_is_pack_i;
