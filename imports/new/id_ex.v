...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_clz_o,
    ...
    // clz 当前保持单周期实现。flush/reset 清零，hold 保持，
    // 使计数 valid 和 rs1 数据同步，不会在流水线停顿时重复或丢失。
    wire ex_is_clz_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_clz_o :
                       ex_is_clz_i;
