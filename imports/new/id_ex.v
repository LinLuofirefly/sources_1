...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_sh1add_o,
    ...
    // sh1add 是单周期写回类指令。flush 时必须清零，hold 时随 ID/EX 一起保持，
    // 防止气泡或停顿后错误重复/丢失该指令的 EX valid。
    wire ex_is_sh1add_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_sh1add_o :
                       ex_is_sh1add_i;
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            ex_is_sh1add_o  <= ex_is_sh1add_next;
