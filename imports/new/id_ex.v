...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_bext_o,
    ...
    // bext 是单周期单比特抽取写回指令。flush/reset 清零，hold 保持，
    // 确保 valid 与 rs1/rs2 数据同拍进入 EX。
    wire ex_is_bext_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_bext_o :
                       ex_is_bext_i;
