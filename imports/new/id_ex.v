...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_clmulh_o,
    ...
    // clmulh 是迭代多周期指令。flush/reset 清零，hold 保持，
    // busy 期间由现有 EX busy 接口冻结前级，只让该新增指令增加 CPI。
    wire ex_is_clmulh_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_clmulh_o :
                       ex_is_clmulh_i;
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            ex_is_clmulh_o  <= ex_is_clmulh_next;
