...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_bexti_o,
    ...
    // bexti 是单周期立即数位提取指令。flush/reset 清零，hold 保持，
    // 保证 valid 与 rs1/shamt 数据在流水寄存器中同步前进。
    wire ex_is_bexti_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_bexti_o :
                       ex_is_bexti_i;
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            ex_is_bexti_o   <= ex_is_bexti_next;
