...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg [2:0]  branch_cond_o,
    (* extract_enable = "no" *) output reg [2:0]  ex_func3_o,
    (* extract_enable = "no" *) output reg        ex_func7_bit5_o,
    (* extract_enable = "no" *) output reg        ex_func7_is_r_o,
    (* extract_enable = "no" *) output reg        ex_func7_is_sub_o,
    (* extract_enable = "no" *) output reg        ex_is_op_imm_o,
    (* extract_enable = "no" *) output reg        ex_is_op_reg_o,
    (* extract_enable = "no" *) output reg        ex_is_branch_o,
    (* extract_enable = "no" *) output reg        ex_is_load_o,
    (* extract_enable = "no" *) output reg        ex_is_store_o,
    (* extract_enable = "no" *) output reg        ex_is_jal_o,
    (* extract_enable = "no" *) output reg        ex_is_jalr_o,
    (* extract_enable = "no" *) output reg        ex_is_auipc_o,
    (* extract_enable = "no" *) output reg        ex_is_lui_o,
    ...
    (* extract_enable = "no" *) output reg        ex_is_xperm4_o,
    ...
    (* extract_enable = "no" *) output reg        ex_ras_predicted_jalr_o
);
    ...
    wire [`BP_GHR_WIDTH-1:0] pred_ghr_next =
    ...
    wire [2:0] branch_cond_next =
        flush_flag_i ? `BR_NONE :
        hold_flag_i  ? branch_cond_o :
                       branch_cond_i;

    wire [2:0] ex_func3_next =
        flush_flag_i ? 3'b0 :
        hold_flag_i  ? ex_func3_o :
                       ex_func3_i;

    wire ex_func7_bit5_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_func7_bit5_o :
                       ex_func7_bit5_i;

    wire ex_func7_is_r_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_func7_is_r_o :
                       ex_func7_is_r_i;

    wire ex_func7_is_sub_next =
        flush_flag_i ? 1'b0 :
    ...
    wire ex_is_op_reg_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_op_reg_o :
                       ex_is_op_reg_i;

    wire ex_is_branch_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_branch_o :
                       ex_is_branch_i;

    wire ex_is_load_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_load_o :
                       ex_is_load_i;

    wire ex_is_store_next =
        flush_flag_i ? 1'b0 :
    ...
    wire ex_is_jalr_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_jalr_o :
                       ex_is_jalr_i;

    wire ex_is_auipc_next =
        flush_flag_i ? 1'b0 :
    ...
    wire ex_is_system_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_system_o :
                       ex_is_system_i;

    wire ex_is_rv32m_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_rv32m_o :
                       ex_is_rv32m_i;

    wire ex_is_csr_op_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_csr_op_o :
                       ex_is_csr_op_i;

    // xperm4 是多周期交叉选择指令。flush/reset 清零，hold 保持，
    // 让 start 与锁存的 rs1/rs2 保持一致，done 后只写回一次。
    wire ex_is_xperm4_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_xperm4_o :
                       ex_is_xperm4_i;
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            branch_cond_o   <= branch_cond_next;
            ...
            ex_is_xperm4_o  <= ex_is_xperm4_next;
