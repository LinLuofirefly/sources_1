...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_andn_o,
    ...
    // andn 是单周期布尔写回类指令。flush 清零，hold 保持，
    // 保证控制位和操作数在流水线停顿/清空时仍严格对齐。
    wire ex_is_andn_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_andn_o :
                       ex_is_andn_i;
