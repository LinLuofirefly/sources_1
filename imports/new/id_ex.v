...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_sh3add_o,
    ...
    // sh3add 是单周期写回类指令，不需要 busy/done。
    // reset/flush 清零，hold 保持，使控制位与同一条指令的数据在 ID/EX 中同步。
    wire ex_is_sh3add_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_sh3add_o :
                       ex_is_sh3add_i;
    ...
    always @(posedge clk) begin
        if (rst == 1'b0) begin
            ...
            ex_is_sh3add_o  <= ex_is_sh3add_next;
