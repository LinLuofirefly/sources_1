...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_orn_o,
    ...
    // orn 是单周期布尔写回类指令。flush/reset 清零，hold 保持，
    // 使该 valid 位不会在流水线停顿或清空时和操作数错位。
    wire ex_is_orn_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_orn_o :
                       ex_is_orn_i;
