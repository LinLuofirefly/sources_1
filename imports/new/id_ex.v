...
module id_ex (
    ...
    (* extract_enable = "no" *) output reg        ex_is_min_o,
    ...
    // min 是单周期比较选择写回指令。flush/reset 清零，hold 保持，
    // 保证 valid 位与同一条指令的两个操作数同步。
    wire ex_is_min_next =
        flush_flag_i ? 1'b0 :
        hold_flag_i  ? ex_is_min_o :
                       ex_is_min_i;
