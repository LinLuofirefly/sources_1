// =============================================================================
// 文件�? open_risc_v.v
// �? �? RISC-V 处理器核心顶层模�?// �? �? 实现一�?6 级流水线�?RV32I 处理器核心�?//
//         流水线结�?
//         ┌────�?  ┌────�?  ┌────�?  ┌─────�?  ┌─────�?  ┌────�?//         �?IF �?�?�?ID �?�?�?EX �?�?�?MEM1�?�?�?MEM2�?�?�?WB �?//         └────�?  └────�?  └────�?  └─────�?  └─────�?  └────�?//
//         为什么是 6 �?(传统 RISC-V 通常�?5 �??
//         由于使用同步 BRAM 作为存储器，数据 RAM 的读取有 1 拍延迟，
//         因此将传统的 MEM 级拆分为 MEM1 (发起读请�? �?MEM2 (接收读数�?�?//
//         冒险处理:
//         - 数据前�?(Forwarding): EX/MEM1 �?EX, MEM1/MEM2 �?EX, MEM2/WB �?EX
//         - Load-Use 冒险: �?HDU 插入 1 拍气泡，然后前�?//         - 控制冒险 (跳转): CTRL 生成 2 拍冲刷信�?(应对同步 ROM 延迟)
// =============================================================================
...
module open_risc_v (
    ...
    output wire [1:0]  mem_rd_mask_o,      // load width: byte/half/word
    ...
    wire [4:0]  id_rd_addr_o;              // ID 输出: 目标寄存器地址
    wire        id_reg_wen;                // ID 输出: 写使�?
    wire [31:0] id_base_addr_o;            // ID 输出: 访存基地址
    wire [31:0] id_addr_offset_o;          // ID 输出: 地址偏移�?
    wire        data_read_en;              // ID 输出: Load 标记
    ...
    wire [3:0]  ex_wd_reg_o;               // EX 输出: Store 写掩�?    wire [31:0] ex_wd_addr_o;              // EX 输出: Store 写地址
    ...
    wire [3:0]  ex_mem_wd_reg_o;           // EX/MEM1 输出: Store 写掩�?    wire [31:0] ex_mem_wd_addr_o;          // EX/MEM1 输出: Store 写地址
    ...
    assign mem_rd_mask_o = (ex_mem_inst_o[14:12] == `INST_LW) ? 2'b10 :
                           ((ex_mem_inst_o[14:12] == `INST_LH) || (ex_mem_inst_o[14:12] == `INST_LHU)) ? 2'b01 :
                           2'b00;
    ...
    wb wb_inst (
        ...
        .inst_o    (),                     // 悬空: WB 之后无下一�?        .rd_addr_o (wb_rd_addr_o),
