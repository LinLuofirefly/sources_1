...
cd "$(dirname "$0")/.."
rm -rf obj_dir_tb_pred_branch_replay_loop

verilator --binary --timing --trace --top-module tb_pred_branch_replay_loop \
  -Wno-fatal \
  -Iimports/new \
  new/tb_pred_branch_replay_loop.sv \
  ...
  --Mdir obj_dir_tb_pred_branch_replay_loop

./obj_dir_tb_pred_branch_replay_loop/Vtb_pred_branch_replay_loop
