 ...
 *
 * The original v3.1.5 header includes rtlibc.h for every GCC target.  This
 * Nano BSP is freestanding and intentionally has no newlib/POSIX layer, so
 * only pull those definitions in when libc support is explicitly enabled.
 */
#ifdef RT_USING_LIBC
#include "rtlibc.h"
#endif
